...
public class BossKeyDoor : MonoBehaviour {
	...
	{
		if (player.name == "Jillian") {
			playerKeyCheck = player.GetComponent<JillianCharacterController> ().bossKeyStat;
		} 
		else 
		{
			playerKeyCheck = player.GetComponent<JohnnyCharacterController> ().bossKeyStat;
		}
